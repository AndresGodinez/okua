...
/**
 * Class GetCfdiXlsxApiViewTest
 * @package Api
 */
class GetCfdiXlsxApiViewTest extends TestCase
{
    /** @var Container */
    ...
    public function testUpAndRunning()
    {
        ...
        $request = TestUtils::makeServerRequestMock('GET', '/api/cfdi/xlsx');
        self::$router->dispatch($request, self::$container->get('response'));
    }

    public function testGetCfdiXlsxSuccessfully()
    {
        $now = new \DateTime();
        $dummyData = [
            'id' => 0,
        ...
        $this->assertEquals($contentType, TestUtils::CONTENT_TYPE_APPLICATION_SPREADSHEET, 'The response CONTENT_TYPE header is not JSON');
