...
class GetBillsTotalApiViewTest extends TestCase
{
    ...
    public function testGetLastRegistersWithLimit()
    {
        ...
        try {
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now, 'email' => 'carlos.hernandez@connectit.com.mx']);
            self::$em->persist($register);

            $now1 = clone $now;
            $now1->modify('-1 day');
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now1, 'email' => 'carlos.hernandez@connectit.com.mx']);
            self::$em->persist($register);
            ...
            $now2 = clone $now1;
            $now2->modify('-1 hour');
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now2, 'email' => 'israel.torres@connectit.com.mx']);
            self::$em->persist($register);

            $now3 = clone $now2;
            $now3->modify('-1 hour');
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now3, 'email' => 'adan.morales@connectit.com.mx']);
            self::$em->persist($register);

            $now4 = clone $now3;
            $now4->modify('-1 hour');
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now4, 'email' => 'adan.morales@connectit.com.mx']);
            self::$em->persist($register);

            $now5 = clone $now4;
            $now5->modify('-1 minute');
            $register = self::$fm->instance(BillInfo::class, ['type' => 'I', 'total' => 500, 'emailDatetime' => $now5, 'email' => 'diego.soto@connectit.com.mx']);
            self::$em->persist($register);
